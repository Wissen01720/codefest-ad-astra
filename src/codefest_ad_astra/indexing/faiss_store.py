...
def _escribir_jsonl_atomico(path: Path, registros: Iterable[dict[str, Any]]) -> None:
    """Escribe a un temporal y hace `os.replace`.

    Una corrida interrumpida no puede dejar un `metadata.jsonl` a medias junto a
    un `index.faiss` completo: eso es exactamente la desalineación silenciosa
    que este módulo trata de imposibilitar.
    """
...
def guardar_base_vectorial(
    ...
    # La metadata primero: si algo falla al serializarla, no queda un índice
    # huérfano apuntando a un archivo viejo.
    _escribir_jsonl_atomico(directorio / NOMBRE_METADATA, metadata)
    faiss.write_index(indice, str(directorio / NOMBRE_INDICE))
